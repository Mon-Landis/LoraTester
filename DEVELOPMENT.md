...
- 布局刷新不是尺寸调整事件。输入文字变化、普通接线变化、工作流分页切换和 Node 2.0 的多轮恢复都会重新调用前端同步函数；这些路径只能重新应用可见性、禁用状态和标签，不能无条件调用 `computeSize()`/`setSize()`。
- 只有动态输入数量或模式确实改变了可见控件，或者警告条确实出现/消失时，才允许重新计算高度。工作流恢复后的节点尺寸属于用户数据，必须保留；延迟到下一帧的旧 `setSize()` 也不能覆盖用户随后进行的手动调整。
- 动态输入数组发生实际变化后才刷新 Node 2.0 的响应式集合。每轮刷新都重新赋值、递增 graph version 或重建 DOM 会造成启动卡顿和节点尺寸反复跳回。
- XY 轴合成器的底部详情只用于解释风格代码映射；顶部条目已经显示权重和代码，不要重复输出完整风格组合。提示词轴不在底部重复渲染提示词正文。
...
11. 布局刷新、输入编辑和接线后是否保留用户手动设置的节点尺寸，启动工作流时是否没有连续重排或卡顿。
